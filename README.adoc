...
**NEW**: `jcli` is now completely configuration-driven! You can add new commands without writing any Java code.
 ...
 **Note on Reflection**: If you add new classes to `commands.yaml` that are not yet known to the native image, you must also add them to `src/main/resources/META-INF/native-image/jcli/jcli/reflect-config.json` so they are accessible at runtime.